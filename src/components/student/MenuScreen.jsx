...
export default function MenuScreen({
  ...
  if (gameState === 'main_select') {
    ...
    return (
      <div className="flex-1 flex flex-col items-center p-6 md:p-12 overflow-y-auto z-10 relative">
        <button onClick={handleExitToTitle} className="absolute top-6 left-6 text-slate-500 font-black flex items-center gap-2">
          <ArrowLeft size={20} /> タイトル
        </button>
            ...
            {MAIN_CATEGORIES.map((cat) => {
              ...
              return (
                <motion.div 
                  ...
                  className={`relative p-1.5 rounded-[32px] shadow-md transition-all ${isAvailable ? 'cursor-pointer ' + cat.bg : 'cursor-not-allowed bg-slate-300 opacity-60'}`}
                >
  ...
  if (gameState === 'sub_select') {
    const subCategories = availableCategories.filter(cat => 
      SUB_MAPPING[activeMain].includes(cat.categoryId) && cat.problems?.length > 0
    );

    return (
      <div className="flex-1 flex flex-col items-center p-6 md:p-12 overflow-y-auto z-10 relative">
        <div className="max-w-4xl w-full">
          <button onClick={backToMain} className="mb-6 text-slate-500 font-black flex items-center gap-2">
  ...
  if (gameState === 'level_select') {
    const levels = [
      { num: 1, name: "Level 1 (初級)", bg: "bg-[#7ee2b8]"}, 
      { num: 2, name: "Level 2 (中級)", bg: "bg-[#fbbf24]"}, 
      { num: 3, name: "Level 3 (上級)", bg: "bg-[#fbb6d6]"}
    ];

    return (
      <div className="flex-1 flex flex-col items-center p-6 md:p-12 overflow-y-auto z-10 relative">
        <div className="max-w-4xl w-full">
          <button onClick={backToSub} className="mb-6 text-slate-500 font-black flex items-center gap-2">
            <ArrowLeft size={20} /> 戻る
          </button>
