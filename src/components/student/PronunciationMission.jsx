...
export default function PronunciationMission({ targetSentence, onComplete }) {
  ...
  useEffect(() => {
    if (accuracy !== null && accuracy >= 80) {
      try {
        ...
        playNote(523.25, now, 0.1);       // C5 (ド)
        playNote(659.25, now + 0.1, 0.1); // E5 (ミ)
        playNote(783.99, now + 0.2, 0.2); // G5 (ソ)
        playNote(1046.50, now + 0.3, 0.5);// C6 (高いド)
      } catch (e) {
  ...
  return (
    <motion.div 
      ...
      className="bg-slate-800 text-white p-4 md:p-5 rounded-3xl shadow-2xl border-4 border-rose-400 mt-2 flex flex-col items-center w-full relative"
    >
      {/* 🌟 派手なアニメーション：80%以上でキラキラが飛び出す！ */}
      <AnimatePresence>
      ...
      <p className="text-slate-400 font-bold mb-3 text-center text-xs md:text-sm">
        正しく発音し終えるまで次の問題には進めません。
      </p>

      {/* ターゲット英文 */}
      <div className="bg-slate-700 p-3 md:p-4 rounded-xl mb-4 w-full text-center">
        <p className="text-xl md:text-2xl font-black text-cyan-300">{targetSentence}</p>
      </div>
